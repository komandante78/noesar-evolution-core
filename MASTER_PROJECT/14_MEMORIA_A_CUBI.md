...
| `CUBE-004` | Le tre semantiche di `05` sono separate da tabelle/tipi diversi — un elemento del Cubo 3 (Corpus) non è restituibile come affermazione del Cubo 1 (Biblioteca) nemmeno per un errore di programmazione | **C** | test di tipo/schema che tenta la confusione e verifica il rifiuto strutturale, non applicativo | ⚠️ **TENSIONE ARCHITETTURALE TROVATA (`D-0261`), non risolta — decisione dell'Owner necessaria**: lo schema concreto proposto in `§9.2` mette **tutti e quattro i cubi in un'unica tabella** `memory_records` con una colonna ENUM `cube` — esattamente ciò che questo criterio Critico vieta ("tabelle diverse", non "stessa tabella con una colonna diversa"). Con lo schema di `§9.2` così com'è, una query che dimentica `WHERE cube = 'library'` restituisce silenziosamente anche righe Corpus — un errore di programmazione che il criterio dichiara di voler rendere impossibile. Costruito comunque perché è la tabella che il resto della fase 9 richiede letteralmente, ma la separazione strutturale (viste tipate per cubo con permessi che vietano la lettura diretta della tabella base, o tabelle realmente separate) è rimandata a una decisione esplicita — vedi `docs/DECISION_LOG.md` `D-0261` |
