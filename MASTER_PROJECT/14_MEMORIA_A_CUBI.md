...
| `CUBE-003` | Un candidato non tracciabile a un evento di sessione non viene emesso dalla compattazione (fallire chiuso, §4.1 regola 3) | **C** | test sulla pipeline di compattazione con un evento sintetico non riconducibile | ✅ **COSTRUITO (`D-0263`)** — `memory-compaction.mjs`: gli eventi della sessione sono quelli di `EventLedger` (append-only, concatenato, uno per run workspace-actions — la stessa fonte già letta da `D-0259`), `ACTION_RULES` è una tabella chiusa e un'azione non riconosciuta non emette MAI un candidato, in modo strutturale (l'emissione è guidata dalla tabella, non da un'inferenza). Provato dal vivo con un evento sintetico `workspace_action.simulated` (non in tabella): scartato, `skipped:1`, zero righe scritte — esattamente il test che questo criterio chiede |
...
| `CUBE-006` | Il richiamo (`recall`) restituisce sempre `coverage` e `not_found`, mai silenziosamente parziale | **A** | test sul contratto `recall()` (§9.4) con un insieme di candidati noto | ✅ **COSTRUITO (`D-0263`)** — `memory-service.mjs::recall()`: `items` sono sempre record (mai prosa), `coverage` viaggia sempre col risultato (`candidates`/`examined`/`returned`/`model`/`vectorIndexComplete` — quest'ultimo onestamente `false` finché nessun modello di embedding è configurato, mai finto), `notFound` è esplicito per ogni criterio richiesto che non ha prodotto nulla. Provato dal vivo: ricerca senza filtri attraverso `all_memories` (più cubi insieme, ognuno col proprio `cube` corretto — nessuna violazione di `CUBE-004`), ricerca filtrata per cubo che non attraversa mai un altro cubo, query senza risultati che dichiara `notFound:["testo di ricerca"]` invece di un array vuoto silenzioso |
...
| `CUBE-008` | Un cambio di modello di embedding non perde record — solo re-indicizza, coi vecchi vettori che continuano a servire finché il nuovo indice non è completo | **A** | procedura di cambio modello (§9.3) misurata end-to-end su un corpus di prova | ✅ **COSTRUITO (`D-0264`)** — `memory-model-swap.mjs`, i quattro passi di `§9.3` uno per metodo: `registerModel()` (`is_current:false`), `backfillBatch()` (incrementale, un record alla volta, tollera un crash a metà lotto), `activate()` (rifiuta sotto il 100% di copertura, poi un solo `BEGIN`/`COMMIT` che sposta `is_current`), `purgeSuperseded()` (rifiuta di cancellare l'indice del modello CORRENTE). Misurato dal vivo su un corpus reale di 7 record: copertura 0→100%, cutover atomico verificato (mai zero né due modelli `is_current`), `recall()` passa onestamente da `vectorIndexComplete:false` a `true` una volta che il nuovo modello è corrente e indicizzato, un secondo modello sostituisce il primo e la pulizia del vecchio indice non tocca `memory_records` (7/7 record intatti). **35/35 PASS**, incluso un bug reale trovato e riparato: `purgeSuperseded()` riportava sempre `deleted:0` perché il client Postgres di questo progetto calcola `rowCount` dal numero di righe RESTITUITE, non dal tag `CommandComplete` — una `DELETE` senza `RETURNING` sembra sempre cancellare zero righe anche quando ne cancella sette |
| `CUBE-009` | L'utente non vede la tassonomia a quattro cubi — una sola destinazione `Memoria`, tre gesti (cerca/sfoglia/approva), parole normali mai `promotion_state`/`contamination`/`cube` | **A** | ispezione della WebUI: nessuna di quelle tre parole compare fuori da una vista di dettaglio esplicitamente richiesta | ✅ **COSTRUITO (`D-0265`) — CHIUDE L'INTERA MATRICE DI ACCETTAZIONE DI QUESTO DOCUMENTO.** Tredicesima destinazione `Memory` nella barra laterale, tre gesti (`GET /api/v1/memory/recall` per cerca/sfoglia, la coda di approvazione a 4 fonti già esistente per approva/scarta — nessuna nuova rotta backend). `MEMORY_CATEGORY_LABELS` traduce le categorie dello schema in parole normali («Decision», «Fact», «Lesson»…); `cube`/`promotion_state`/`contamination` mai visibili fuori da un `<details>` aperto di proposito. Provato **dal vivo in un Chromium reale via Puppeteer**: suite E2E completa **334/334 PASS**, audit di accessibilità **27/27 PASS**, entrambe camminano su `Memory` come una delle tredici destinazioni. **Due bug reali trovati e riparati verificando questo dal vivo, nessuno nella destinazione Memory stessa** — un click Puppeteer che si staccava dal documento durante un re-render (`scrollIntoViewIfNeeded`, riparato con un click sintetico in-pagina) e una corsa fra il test e il proprio refresh asincrono della striscia di approvazione (riparata con un'attesa esplicita) — entrambi mascherati finché la coda di approvazione era sincrona-veloce in memoria, esposti dal vero giro di rete Postgres che `D-0263` ha introdotto. Vedi `docs/DECISION_LOG.md` `D-0265` per il dettaglio completo, inclusa la collisione di nomi trovata e risolta (`id="view-memory"` apparteneva già al pannello manuale "Notes", ora rinominato) |
