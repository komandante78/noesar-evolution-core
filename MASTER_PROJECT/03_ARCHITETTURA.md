...
| `ARCH-005` | Nessun adattatore (`ModelRuntimeAdapter`, `VectorStoreAdapter`, ecc.) può auto-concedersi permessi — un manifest è una richiesta, i token li emette il motore | **C** | test che un adattatore chiede un permesso non concesso e viene rifiutato | ⚠ **parziale (`D-0244`)**: `local-model-runtime.mjs` è l'unico adattatore che esiste oggi, e il suo unico metodo privilegiato (`launch()`, che genera un processo OS reale) ora richiede un token speso attraverso lo stesso `TokenMinter` di `workspace-actions.mjs` (`adapter-capability.mjs`, `ADAPTER_MANIFESTS`) — verificato con 26 test, inclusi HTTP adversarial contro le route reali. `attach()`/`complete()`/`configure()` sullo stesso adattatore restano scoperti (nessun Network Egress Broker esiste ancora), e gli altri sei adattatori del documento `03 §4` non esistono ancora, quindi i loro manifest non sono scritti |
...
| `ARCH-008` | Isolamento per capacità: ogni capacità gira coi limiti scritti nel proprio token, non con quelli del container intero | **A** | misura dei limiti effettivi (cgroup) applicati per sandbox vs per container | ⏳ **investigato concretamente, non solo dichiarato (`D-0246`)**: Landlock bloccato dal kernel dell'host (`ENOSYS`, `CONFIG_SECURITY_LANDLOCK` non compilato — non risolvibile da codice), i cgroup v2 sono montati ma non delegati in scrittura dal container runtime (stessa causa, non risolvibile da codice), il self-narrowing seccomp è confermato disponibile ma richiede un vero esecutore sandbox a processo separato per spesa di token — oggi `executor.mjs` e il gate degli adattatori (`D-0244`) agiscono entrambi in-process. Prerequisito architetturale nominato, non costruito; due opzioni per l'Owner in `D-0246` |
