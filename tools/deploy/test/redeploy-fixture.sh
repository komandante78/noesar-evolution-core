...
echo "=== 11. --set / --unset: correcting what STEP 1 copies forward — F-ROT-001 ==="
# The finding this exists for: the environment is read back from the container being replaced, so
# a wrong value survives every deploy. `NOESAR_ALLOWED_HOSTS` named a container IP that had not
# existed since a rotation, and a hand-edit would have been copied over by the next deploy.
new_world setunset
run_tool -- --apply --authorized-by-owner --set NODE_ENV=staging --set BRAND_NEW=hello --unset FILLER_7,FILLER_8
[ "$(code)" = "0" ] && pass "set/unset: exit 0" || fail "set/unset: exit 0" "got $(code)"
[ "$(envof inst NODE_ENV)" = "staging" ] && pass "set: an existing variable is corrected" || fail "set: existing corrected" "$(envof inst NODE_ENV)"
[ "$(envof inst BRAND_NEW)" = "hello" ] && pass "set: a variable that did not exist is added" || fail "set: new added" "absent"
[ -z "$(envof inst FILLER_7)" ] && [ -z "$(envof inst FILLER_8)" ] && pass "unset: both named variables are gone" || fail "unset: gone" "one survived"
# 24 in, minus the two unset, plus the one added.
[ "$(wc -l < "$FAKE_ROOT/c/inst/env")" = "23" ] && pass "set/unset: the count moved by exactly the change asked for" || fail "set/unset: count" "$(wc -l < "$FAKE_ROOT/c/inst/env")"
[ "$(envof inst FILLER_1)" = "value1" ] && pass "set/unset: untouched variables are untouched" || fail "set/unset: others intact" "changed"
[ "$(envof inst SECRET_A)" = "$OLD_SYNTH" ] && pass "set/unset: secrets are carried over, not disturbed" || fail "set/unset: secrets intact" "changed"
pred >/dev/null && pass "set/unset: predecessor preserved ($(pred))" || fail "set/unset: predecessor" "absent"

# A value in argv is readable in `ps` by every user on the host for as long as the process lives —
# property 4. Secrets are GENERATED by --rotate-secret, never typed, so this flag refuses to carry
# one, and refuses BEFORE anything is touched.
new_world setsecret
run_tool -- --apply --authorized-by-owner --set SECRET_A=leaked
