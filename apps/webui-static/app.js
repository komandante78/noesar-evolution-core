...
// Native prompt()/confirm() block the whole page (measured: one froze the UI for ~3 minutes,
// see CHIUSURA_20260908_NOTTE.md) and are invisible to the translator and to any automated
// walk of the page. <dialog> is the platform-native, non-blocking replacement: no overlay or
// focus-trap code of our own, the browser provides both, and plain input/textarea/button
// inside it already pick up the theme via the existing global element styles.
function appDialogEl(){
  let el=document.getElementById('appDialog');
  if(!el){el=document.createElement('dialog');el.id='appDialog';el.className='app-dialog';document.body.appendChild(el);}
  return el;
}
function textDialog(message,defaultValue='',{multiline=false,password=false}={}){
  return new Promise((resolve)=>{
    const el=appDialogEl();
    const field=multiline?`<textarea id="appDialogInput" rows="8">${escapeHtml(defaultValue)}</textarea>`:`<input type="${password?'password':'text'}" id="appDialogInput" value="${escapeHtml(defaultValue)}">`;
    el.innerHTML=`<form method="dialog"><p>${escapeHtml(message)}</p>${field}<menu><button value="cancel">${escapeHtml(t('Cancel'))}</button><button value="ok" class="primary">${escapeHtml(t('OK'))}</button></menu></form>`;
    el.addEventListener('close',()=>resolve(el.returnValue==='ok'?el.querySelector('#appDialogInput').value:null),{once:true});
    el.showModal();
    const input=el.querySelector('#appDialogInput');input.focus();if(!multiline)input.select();
  });
}
function confirmDialog(message){
  return new Promise((resolve)=>{
    const el=appDialogEl();
    el.innerHTML=`<form method="dialog"><p>${escapeHtml(message)}</p><menu><button value="cancel">${escapeHtml(t('Cancel'))}</button><button value="ok" class="primary" autofocus>${escapeHtml(t('OK'))}</button></menu></form>`;
...
function optionList(items,{empty='None',label=(item)=>item.name,value=(item)=>item.id,selected=null}={}){return `<option value="">${escapeHtml(empty)}</option>${items.map((item)=>`<option value="${escapeHtml(value(item))}" translate="no" ${value(item)===selected?'selected':''}>${escapeHtml(label(item))}</option>`).join('')}`;}
...
function renderPrivacyDisclosures(disclosures,telemetry){
  ...
  // The panel is on every view; clicking its title folds the detail rows away (the title and
  // the Revoke button stay, so the disclosure itself does not disappear). Choice is per-browser.
  try{panel.classList.toggle('collapsed',localStorage.getItem('noesar.privacyDisclosures.collapsed')==='1');}catch{}
...
function renderHome(){$('#homeProjects').innerHTML=state.projects.slice(0,5).map((item)=>`<article><div><b translate="no">${escapeHtml(item.name)}</b><small translate="no">${escapeHtml(item.description||t('No description'))}</small></div></article>`).join('')||'No projects yet.';$('#homeConversations').innerHTML=state.conversations.slice(-5).reverse().map((item)=>`<article><div><b translate="no">${escapeHtml(item.title)}</b><small translate="no">${escapeHtml(item.mode)}</small></div></article>`).join('')||'No conversations yet.';}
...
function renderProjects(){
  ...
  $('#projectList').innerHTML=state.projects.map((item)=>`<article class="entity-card"><h3 translate="no">${escapeHtml(item.name)}</h3><p translate="no">${escapeHtml(item.description)}</p><small translate="no">${escapeHtml(item.tags.join(' · '))}</small><div class="card-actions"><button translate="no" data-select-project="${escapeHtml(item.id)}">${escapeHtml(t('Use project'))}</button> <button type="button" translate="no" class="danger" data-delete-project="${escapeHtml(item.id)}">${escapeHtml(t('Delete'))}</button></div><div class="project-delete-confirm" data-delete-project-panel="${escapeHtml(item.id)}" hidden></div></article>`).join('')||t('No projects.');
...
// One writer for the inspector, and it marks what it writes as untranslatable: the pane
// holds a JSON document — provider ids, model names, message ids — and the translator walks
// every text node in the page. Its empty sentence stays translatable because the mark goes
// on only when the JSON does.
function dumpContext(value){const el=$('#contextInspector');if(!el)return;el.setAttribute('translate','no');el.textContent=JSON.stringify(value,null,2);}
async function inspectContext(){if(!state.activeConversationId)return;const data=await api(`/api/v1/conversations/${state.activeConversationId}/context?branchId=${state.activeBranchId}`);$('#tokenEstimate').textContent=`≈ ${data.tokenEstimate} ${t('tokens')}`;$('#chatContextSummary').textContent=`${data.messages.length} ${t('messages')} · ${data.memories.length} ${t('memories')} · ${data.sources.length} ${t('sources')} · ${data.tools.length} ${t('tools')}`;dumpContext({mode:data.conversation.mode,providerId:data.providerId,model:data.model,project:data.project?.name,included:data.included,tokenEstimate:data.tokenEstimate});}
...
function renderTasks(){
  ...
  const chip=$('#taskZoneChip');if(chip){chip.setAttribute('translate','no');chip.textContent=`${t('Times in')} ${zoneName()}`;}
  ...
  if(formZone){formZone.setAttribute('translate','no');formZone.textContent=`${t('Schedule and Due are read in')} ${zoneName()}${t(', and stored as an instant.')}`;}
...
function renderArtifacts(){$('#artifactList').innerHTML=state.artifacts.map((item)=>`<article class="entity-card"><h3 translate="no">${escapeHtml(item.title)}</h3><small translate="no">${escapeHtml(t(item.type))} · ${t('version')} ${item.versions.length}</small><pre>${escapeHtml(item.versions.at(-1)?.content??'')}</pre><button data-edit-artifact="${item.id}">New version</button></article>`).join('')||'No documents.';$$('[data-edit-artifact]').forEach((button)=>button.addEventListener('click',async()=>{const item=state.artifacts.find((a)=>a.id===button.dataset.editArtifact);const content=await textDialog('New artifact version',item.versions.at(-1)?.content??'',{multiline:true});if(content!==null){await api(`/api/v1/artifacts/${item.id}`,{method:'PATCH',body:JSON.stringify({content})});await refreshWorkspace();}}));}
...
function renderSources(){$('#sourceList').innerHTML=state.sources.map((item)=>`<article class="entity-card"><h3 translate="no">${escapeHtml(item.name)}</h3><p translate="no">${escapeHtml(item.mimeType)}</p><small translate="no">${escapeHtml(t(item.extractionStatus))} · ${item.byteLength} ${t('bytes')}</small>${item.piiRedacted?`<div><span class="badge">${escapeHtml(t('Identifiers redacted'))}</span></div>`:''}<button data-preview-source="${item.id}">Preview passages</button>${item.piiRedacted?`<button data-reveal-source="${item.id}" data-source-name="${escapeHtml(item.name)}">${escapeHtml(t('Reveal original'))}</button>`:''}<button class="danger" data-delete-source="${item.id}" data-source-name="${escapeHtml(item.name)}">Delete</button></article>`).join('')||'No sources indexed yet — add a file or paste text to make it searchable.';$$('[data-preview-source]').forEach((button)=>button.addEventListener('click',async()=>{const item=await api(`/api/v1/sources/${button.dataset.previewSource}`);$('#knowledgeResults').innerHTML=item.passages?.map((passage)=>`<article class="entity-card"><h3 translate="no">${t('Passage')} ${passage.index}</h3><p>${escapeHtml(passage.text)}</p></article>`).join('')||'<p>No extracted passages.</p>';}));$$('[data-reveal-source]').forEach((button)=>button.addEventListener('click',async()=>{const password=await textDialog(t('Enter your password to reveal the unredacted original.'),'',{password:true});if(password===null)return;await withBusy(button,async()=>{let result;try{result=await api(`/api/v1/sources/${button.dataset.revealSource}/reveal`,{method:'POST',body:JSON.stringify({password})});}catch(error){setStatus(error.message,true);return;}if(result.originalText)await textDialog(t('Unredacted original'),result.originalText,{multiline:true});if(result.blobBase64){const bytes=Uint8Array.from(atob(result.blobBase64),(c)=>c.charCodeAt(0));const blob=new Blob([bytes],{type:result.mimeType||'application/octet-stream'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=result.blobFilename||button.dataset.sourceName||'document';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);}});}));$$('[data-delete-source]').forEach((button)=>button.addEventListener('click',async()=>{const name=button.dataset.sourceName;if(!(await confirmDialog(`${t('Delete this document?')}\n\n${name}\n\n${t('This permanently removes the file and everything extracted from it. This cannot be undone.')}`)))return;if(!(await confirmDialog(t('Are you sure? There is no way to recover this after deletion.'))))return;await withBusy(button,async()=>{await api(`/api/v1/sources/${button.dataset.deleteSource}`,{method:'DELETE',body:'{}'});await refreshWorkspace();});}));}
...
$('#sourceRedactLabel').textContent='Redact identifiers (codice fiscale, IBAN, email, phone) before indexing — names and addresses are not covered';
...
$('#sourceForm').addEventListener('submit',async(event)=>{event.preventDefault();try{const redact=$('#sourceRedact').checked;const file=$('#sourceBinary').files[0];if(file){setStatus(`Extracting ${file.name} locally…`);await api('/api/v1/sources/upload',{method:'POST',body:JSON.stringify({projectId:$('#sourceProject').value||null,name:file.name,mimeType:file.type||$('#sourceMime').value,bytesBase64:await fileToBase64(file),redact})});}else{await api('/api/v1/sources',{method:'POST',body:JSON.stringify({projectId:$('#sourceProject').value||null,name:$('#sourceName').value,mimeType:$('#sourceMime').value,text:$('#sourceText').value,redact})});}event.target.reset();$('#sourceMime').value='text/plain';$('#sourceRedact').checked=true;await refreshWorkspace();setStatus('Source ingested and indexed.');}catch(error){setStatus(error.message,true);}});
...
function renderProviders(){
  ...
  const runtimeProviderCard=(item)=>`<article class="entity-card provider-card"><h3 translate="no">${escapeHtml(item.name)}</h3>`
    +`<p translate="no">${escapeHtml(item.type)} · ${escapeHtml(item.baseUrl)}</p>`
...
function agentStatusBadge(agentId){
  ...
  return `<p><span class="badge" translate="no">${escapeHtml(t('Run in progress'))}</span></p>`;
}
...
async function runWorkspaceAction(kind,opts={}){
  ...
  try{
    if(kind==='simulate'){
      ...
      const reason='reason' in opts?opts.reason:((await textDialog('Reason for rejecting this plan (optional):'))??null);
...
function bindApprovalActions(){
  const decide=async(id,decision)=>{
    const reason=decision==='reject'?(await textDialog('Reason for rejecting:'))??'':'';
    try{
...
function renderBenchNavigator(){
  const list=(items,label,empty,destination)=>items.length
    ?items.slice(0,6).map((item)=>`<button type="button" translate="no" data-jump="${escapeHtml(destination)}" title="${escapeHtml(label(item))} — ${escapeHtml(t('opens'))} ${escapeHtml(destination)}">${escapeHtml(label(item))}</button>`).join('')
  ...
  if(preview&&empty){
    if(conversationMessages?.length){
      ...
      preview.innerHTML=conversationMessages.slice(-4).map((message)=>
        `<p class="coden-conversation-turn" translate="no"><b>${message.role==='user'?escapeHtml(t('You')):escapeHtml(t(message.role))}</b> ${escapeHtml(message.content.length>140?`${message.content.slice(0,140)}…`:message.content)}</p>`
      ).join('');
    }else{
...
function renderResearchOutcome(outcome){
  ...
  if(outcome.outcome==='REFUSE'){
    ...
    $('#researchContestButton').addEventListener('click',(event)=>withBusy(event.currentTarget,async()=>{
      ...
      const note=await textDialog(t('Why should this be reconsidered?'));
      if(note===null)return;
...
async function loadInstallableModels(){
  ...
  try{
    ...
    for(const file of files){
      ...
      const name=document.createElement('span');name.setAttribute('translate','no');name.textContent=`${file.name}${size}`;
      row.append(name);
      if(file.installed){
        const done=document.createElement('span');done.className='badge badge-on';done.textContent=t('Installed');
        row.append(done);
      }else{
        const install=document.createElement('button');install.type='button';install.setAttribute('translate','no');install.textContent=t('Install this one');
