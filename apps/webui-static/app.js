...
function textDialog(message,defaultValue='',{multiline=false}={}){
  return new Promise((resolve)=>{
    const el=appDialogEl();
    const field=multiline?`<textarea id="appDialogInput" rows="8">${escapeHtml(defaultValue)}</textarea>`:`<input type="text" id="appDialogInput" value="${escapeHtml(defaultValue)}">`;
