...
export const RUNTIME_ONLY = Object.freeze([
  ...
  // The Installed tools and Models panels on the home. Every line of both is written by
  // `renderTools()` and `renderModels()` out of what /api/v1/home sends, so none of it can
  // be found in the markup: both panels rendered in English on an Italian page until
  // 2026-09-08, and the static check reported no gap because there was no markup to read.
  // The three `reach` values are this product's own vocabulary for where something runs,
  // not data, so they are translated; a tool's host, a provider's base URL and a model id
  // are data, and are marked `translate="no"` where they render.
  'Reaches',
  'in process',
  'this machine',
  'off machine',
  ...
  'Showing',
  'registered tools.',
  'Provenance here is where a tool points, not who added it: the record carries no registrar. That name is in the audit log under',
  'not configured',
  'launched by this installation',
  'attached, not launched here',
  'overridden by the environment',
  'mutative',
  'model_descriptors.trust_state exists in the schema and no code writes it. Rendering a value nothing sets would be a number with no source.',
  // The privacy footer, the approval strip, the product metric and the two timezone notes.
  // The privacy banner is server prose: `privacy.mjs` picks one of five headline/detail
  // pairs, and only the local-only pair had an entry — so the notice was translated on an
  // installation with nothing configured and English on every other, which is exactly the
  // installation that needs to read it. Measured 2026-09-08 with a provider configured.
  // The metric and the two zone notes are sentences broken by a value, and the halves are
  // put through `t()` where they are written.
  'Privacy state not confirmed.',
  'The configuration has not been read, so no guarantee is being made either way.',
  'An external request was blocked by policy.',
  'Nothing was sent. Review the audit trail for the refused request.',
  'An external connection is configured but not active.',
  'Consent has not been granted, so nothing has been sent yet.',
  'External connection active or requested.',
  'Review the exact destination and data scope before continuing.',
  'Privacy state could not be confirmed.',
  'The server did not answer the privacy check, so this notice is not reporting a verified state.',
  '● External by consent',
  '● Privacy state not confirmed',
  ...
  'Measured in',
  'seconds of human review per decided change',
  'Rejected changes are counted, not excluded (UI-072). Interval starts at:',
  'the shadow result was ready for a human (pre-D-0567 samples: the approval was raised)',
  // The chat. The speaker label on each message, the two buttons under it, the branch and
  // route pickers, the context summary and the two panels that say nothing has happened
  // yet — all written by `renderMessages()`, `renderProviders()` and `inspectContext()`,
  // none of it in the markup. Measured 2026-09-08 on a real conversation: the whole page
  // in Italian and every one of these in English.
  'No branch',
  'Fork here',
  'Remove from context',
  'No source has been cited in this chat yet.',
  'No work has been started from this chat yet.',
  ...
  'Commands, with or without /: status | plan <what to do> | measure <runId> | get <runId> | approve <runId> | reject <runId> [reason] | restore <runId> | events <runId> | map [path] | search <query> | simulate <runId> | help\nThe same live session as the workbench and the full terminal: a plan made here is the same run everywhere.',
  'Unknown command',
  'Tools used',
  'Type `help`.',
...
const it = {
  'Type the code on screen, wait for it to change, then type the new one and confirm within thirty seconds. Two consecutive codes prove the clock on your phone agrees with this server; a single code can pass by luck and fail at your next sign-in.': 'Scrivi il codice che vedi, aspetta che cambi, poi scrivi quello nuovo e conferma entro trenta secondi. Due codici consecutivi dimostrano che l’orologio del telefono va d’accordo con questo server; un codice solo può passare per fortuna e fallire al prossimo accesso.',
  ...
  'Reaches': 'Raggiunge',
  'in process': 'nel processo',
  'this machine': 'questa macchina',
  'off machine': 'fuori da questa macchina',
  'declares itself external': 'si dichiara esterno',
  'part of this product': 'parte di questo prodotto',
  'consent granted': 'consenso concesso',
  'no consent': 'nessun consenso',
  'credential configured': 'credenziale configurata',
  'no credential': 'nessuna credenziale',
  'registered': 'registrato',
  'Showing': 'Mostrati',
  'registered tools.': 'strumenti registrati.',
  'Provenance here is where a tool points, not who added it: the record carries no registrar. That name is in the audit log under': 'Qui la provenienza dice dove punta uno strumento, non chi lo ha aggiunto: il registro non porta il nome di chi lo ha registrato. Quel nome sta nel giornale di controllo, sotto',
  'not configured': 'non configurato',
  'launched by this installation': 'avviato da questa installazione',
  'attached, not launched here': 'collegato, non avviato qui',
  'overridden by the environment': 'scavalcato dall’ambiente',
  'mutative': 'modifica lo stato',
  'model_descriptors.trust_state exists in the schema and no code writes it. Rendering a value nothing sets would be a number with no source.': 'model_descriptors.trust_state esiste nello schema e nessun codice lo scrive. Mostrare un valore che nessuno imposta sarebbe un numero senza sorgente.',
  'Privacy state not confirmed.': 'Stato della riservatezza non confermato.',
  'The configuration has not been read, so no guarantee is being made either way.': 'La configurazione non è stata letta, quindi non viene data alcuna garanzia, né in un senso né nell’altro.',
  'An external request was blocked by policy.': 'Una richiesta verso l’esterno è stata bloccata dalle regole.',
  'Nothing was sent. Review the audit trail for the refused request.': 'Non è stato inviato nulla. Controlla il giornale di controllo per la richiesta rifiutata.',
  'An external connection is configured but not active.': 'Una connessione esterna è configurata ma non è attiva.',
  'Consent has not been granted, so nothing has been sent yet.': 'Il consenso non è stato concesso, quindi finora non è stato inviato nulla.',
  'External connection active or requested.': 'Connessione esterna attiva o richiesta.',
  'Review the exact destination and data scope before continuing.': 'Controlla la destinazione esatta e quali dati escono, prima di proseguire.',
  'Privacy state could not be confirmed.': 'Non è stato possibile confermare lo stato della riservatezza.',
  'The server did not answer the privacy check, so this notice is not reporting a verified state.': 'Il server non ha risposto al controllo sulla riservatezza, quindi questo avviso non riporta uno stato verificato.',
  '● External by consent': '● Esterno per consenso',
  '● Privacy state not confirmed': '● Stato della riservatezza non confermato',
  'Nothing is waiting for a decision.': 'Non c’è nulla in attesa di una decisione.',
  'Times in': 'Orari in',
  'essential': 'essenziale',
  'No branch': 'Nessun ramo',
  'Fork here': 'Dirama da qui',
  'Remove from context': 'Togli dal contesto',
  'No source has been cited in this chat yet.': 'In questa chat non è ancora stata citata alcuna fonte.',
  'No work has been started from this chat yet.': 'Da questa chat non è ancora stato avviato alcun lavoro.',
  ...
  'seconds of human review per decided change': 'secondi di revisione umana per modifica decisa',
  'Rejected changes are counted, not excluded (UI-072). Interval starts at:': 'Le modifiche respinte sono contate, non escluse (UI-072). L’intervallo parte da:',
  'the shadow result was ready for a human (pre-D-0567 samples: the approval was raised)': 'il risultato in ombra era pronto per una persona (campioni precedenti a D-0567: l’approvazione era stata sollevata)',
  'Coverage: —': 'Copertura: —',
  ...
  'Commands, with or without /: status | plan <what to do> | measure <runId> | get <runId> | approve <runId> | reject <runId> [reason] | restore <runId> | events <runId> | map [path] | search <query> | simulate <runId> | help\nThe same live session as the workbench and the full terminal: a plan made here is the same run everywhere.': 'Comandi, con o senza /: status | plan <cosa fare> | measure <runId> | get <runId> | approve <runId> | reject <runId> [motivo] | restore <runId> | events <runId> | map [percorso] | search <query> | simulate <runId> | help\nLa stessa sessione viva del banco e del terminale completo: un piano fatto qui è la stessa corsa ovunque.',
  'Unknown command': 'Comando sconosciuto',
  'Tools used': 'Strumenti usati',
  ...
  'Your own account: your password, the authenticator, and the sessions open right now. Nothing here touches anybody else — other people live under People and access.': 'Il tuo account: la tua password, l’autenticatore e le sessioni aperte adesso. Qui non si tocca nessun altro — le altre persone stanno in Persone e accessi.',
  'The authenticator is optional: this installation signs you in with a username and a password, and the app adds a six-digit code on top. Turning it ON asks for your password alone, because you have no code to give yet. Turning it OFF asks for the password AND a live code, so that a session somebody found unlocked cannot remove it. Enrolment wants two CONSECUTIVE codes: type the one on screen, wait for it to change, type the new one, and confirm within thirty seconds — that pair proves the clock on your phone agrees with this server, where a single code can pass by luck and then fail at your next sign-in. The recovery codes appear once, are kept only as fingerprints, and are the only way back in if the phone is lost.': 'L’autenticatore è facoltativo: questa installazione ti fa entrare con nome utente e password, e l’app aggiunge un codice di sei cifre sopra di essi. Attivarlo chiede la sola password, perché non hai ancora nessun codice da dare. Disattivarlo chiede la password E un codice vivo, così che una sessione trovata aperta non possa toglierlo. L’iscrizione vuole due codici CONSECUTIVI: scrivi quello che vedi, aspetta che cambi, scrivi il nuovo e conferma entro trenta secondi — quella coppia dimostra che l’orologio del telefono va d’accordo con questo server, mentre un codice solo può passare per fortuna e poi fallire al prossimo accesso. I codici di recupero compaiono una volta sola, sono conservati solo come impronte, e sono l’unica via di rientro se perdi il telefono.',
