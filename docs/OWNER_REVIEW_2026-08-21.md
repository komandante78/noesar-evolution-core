...
| 12 | menu `/` (chat e CodeN, web e tui) | entrambe | segnalato più volte dall'Owner come "non fatto come Claude Code" — **misurato, non più letto soltanto**: `claude-in-chrome` non è disponibile in questa sessione (estensione non configurata), quindi ho controllato l'unico modo disponibile — la suite e2e di questo stesso progetto (`tools/browser-e2e.mjs`, ~3800 righe). Il menu "/" del banco CodeN (`#codenMenu`) è testato a fondo e dal vivo, 17 comandi, tastiera, invio. **Il menu "/" della chat semplice (`#chatCommands`, `apps/webui-static/app.js:1255`) non ha NESSUNA prova dal vivo in tutta la suite** — mai verificato in un browser reale, in nessuna sessione precedente | prossimo passo: scrivere e far girare una prova dal vivo per `#chatCommands`, così si scopre il difetto reale invece di continuare a leggere codice |
| | | | | |
