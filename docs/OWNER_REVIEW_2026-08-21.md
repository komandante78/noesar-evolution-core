...
| 12 | menu `/` (chat, web) | web | **CHIUSO in questa sessione, misurato dal vivo — commit `4c6ffe5`**. `claude-in-chrome` non disponibile (estensione non configurata); ho scritto 5 controlli veri nella suite e2e di questo progetto (`tools/browser-e2e.mjs`) e li ho fatti girare su un'installazione usa-e-getta: apre con "/" solo (29 comandi), digitare filtra, freccia giù sposta l'evidenziazione, Tab completa senza mandare, Escape chiude. **Risultato: tutti e 5 PASS.** Suite intera: 516 PASS, 1 FAIL (il gap i18n già tracciato `F-I18N-002`, non collegato). Il menu **funzionava già** — mancava la prova, non la correzione | se dal vivo continua a sembrare sbagliato, non è questo meccanismo: serve dire ESATTAMENTE cosa si vede (aspetto? posizione? un gesto diverso?) — non ho più un'ipotesi di codice da controllare |
| | | | | |
