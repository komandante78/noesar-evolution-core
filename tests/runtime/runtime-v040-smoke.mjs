...
const workspace = mkdtempSync(join(os.tmpdir(), 'noesar-runtime-v040-'));
...
try {
  ...
  if (preflight.version !== '0.4.0') throw new Error('preflight version mismatch');
  if (preflight.correctedMigrationBaseline !== true) {
    throw new Error('corrected migration baseline missing');
  }
  if (preflight.legacyV030Rejected !== true) {
    throw new Error('legacy V0.3.0 was not rejected');
  }
  if (preflight.migrationCount !== 8) throw new Error('migration count mismatch');
  if (preflight.productionReady !== false) {
    throw new Error('preflight falsely production-ready');
  }

  const health = await request('/healthz');
  if (health.data.version !== '0.4.0-runtime-implementation') {
    throw new Error('runtime version mismatch');
  }
  if (health.data.runtime?.migrationBaseline !== '0.4.0') {
    throw new Error('runtime migration baseline mismatch');
  }
  if (health.data.runtime?.productionReady !== false) {
    throw new Error('health falsely production-ready');
  }
  if (health.data.authority.protocolReferenceImplemented !== true) {
    throw new Error('authority protocol reference not reported');
  }
  if (health.data.authority.authenticatedTransportActive !== false) {
    throw new Error('authority transport falsely active');
  }
  if (health.data.dataPlane.repositoryAdapterImplemented !== true) {
    throw new Error('repository adapter not reported');
  }
  ...
  if (after.data.runtime?.migrationBaseline !== '0.4.0') {
    throw new Error('readiness migration baseline mismatch');
  }

  const bootstrap = await request('/api/v1/bootstrap');
  if (bootstrap.status !== 200) throw new Error('bootstrap failed');
  if (bootstrap.data.product.version !== '0.4.0') {
  ...
  console.log('RUNTIME_V040_SMOKE=PASS');
} finally {
